...
# Archivos esperados
FILES = [
    "resultadosLineal.csv",
    "resultadosBinaria.csv",
    "resultadosGalopante.csv",
    "resultadosPosicionLineal.csv",
    "resultadosPosicionBinaria.csv",
    "resultadosPosicionGalopante.csv",
]

# Títulos bonitos por archivo
TITLE = {
    "resultadosLineal.csv": ("Búsqueda lineal", "Tamaño de la secuencia (n)"),
    "resultadosBinaria.csv": ("Búsqueda binaria", "Tamaño de la secuencia (n)"),
    "resultadosGalopante.csv": ("Búsqueda galopante", "Tamaño de la secuencia (n)"),
    "resultadosPosicionLineal.csv": ("Búsqueda lineal", "Posición consultada (índice)"),
    "resultadosPosicionBinaria.csv": ("Búsqueda binaria", "Posición consultada (índice)"),
    "resultadosPosicionGalopante.csv": ("Búsqueda galopante", "Posición consultada (índice)"),
}
...
def pick_xy(df):
    """Elige columnas X e Y (X: n/size/pos, Y: t_mean) de forma flexible."""
...
def plot_one(csv_name):
    ...
    # Etiquetas
    title, xlabel = TITLE.get(csv_name, ("Experimento", "X"))
    ylabel = f"Tiempo promedio por búsqueda ({UNIT})"

    # Gráfico
    plt.figure(figsize=(8, 5))
    plt.plot(df[xcol].values, df[ycol].values, linewidth=1.5, label="t_mean (por punto)")
